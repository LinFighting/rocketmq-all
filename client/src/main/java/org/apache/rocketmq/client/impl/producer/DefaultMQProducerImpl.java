...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    private SendResult sendDefaultImpl(
        ...
        // 校验 Producer 处于运行状态
        this.makeSureStateOK();
        // 校验消息格式（topic、msg长度校验等）
        Validators.checkMessage(msg, this.defaultMQProducer);
        // 随机调用编号（用于下面打印日志，标记为同一次发送消息）
        final long invokeID = random.nextLong();
        long beginTimestampFirst = System.currentTimeMillis();
        long beginTimestampPrev = beginTimestampFirst;
        long endTimestamp = beginTimestampFirst;
        // 获得topic的发送路由信息，如果为空则通过netty从namesrv上获取（TODO 重要方法）
        ...
        if (topicPublishInfo != null && topicPublishInfo.ok()) {
            ...
            // 发送的总次数（同步为2+1）；其他只发送1次
            int timesTotal = communicationMode == CommunicationMode.SYNC ? 1 + this.defaultMQProducer.getRetryTimesWhenSendFailed() : 1;
            // 第几次发送
            int times = 0;
            // 存储每次发送消息选择的broker名
            String[] brokersSent = new String[timesTotal];
            // 循环调用发送消息，直到成功（主要是同步发送使用）
            for (; times < timesTotal; times++) {
                String lastBrokerName = null == mq ? null : mq.getBrokerName();
                // 选择消息要发送到的队列，该选择是根据系统的负载定义的选择器（TODO 重要方法）
    ...
    private TopicPublishInfo tryToFindTopicPublishInfo(final String topic) {
        // 先从本地内存数据结构中获取，系统启动后，新的topic肯定是空
        TopicPublishInfo topicPublishInfo = this.topicPublishInfoTable.get(topic);
        // 当无可用的 Topic发布信息时，从Namesrv获取一次
        if (null == topicPublishInfo || !topicPublishInfo.ok()) {
            //首先将topic的数据结构放置到当前的内存数据结构中
            this.topicPublishInfoTable.putIfAbsent(topic, new TopicPublishInfo());
            //根据topic名称更新topic的路由信息，底层是基于netty的通信，请求nameserv服务
            //如果有数据变化，下层服务会调用当前服务的更新操作
        ...
        if (topicPublishInfo.isHaveTopicRouterInfo() || topicPublishInfo.ok()) {
            ...
            // 用于 Topic发布信息不存在 && Broker支持自动创建Topic的情况
    ...
    public TransactionSendResult sendMessageInTransaction(final Message msg,
        ...
        // 如果延迟消息清空延迟级别（事务消息无延迟）
        if (msg.getDelayTimeLevel() != 0) {
            MessageAccessor.clearProperty(msg, MessageConst.PROPERTY_DELAY_TIME_LEVEL);
        }
        //简单检查消息的对象，消息的topic 消息的body是否有数据，且长度格式合规
        Validators.checkMessage(msg, this.defaultMQProducer);

        SendResult sendResult = null;
        //标识该消息是事务消息
        MessageAccessor.putProperty(msg, MessageConst.PROPERTY_TRANSACTION_PREPARED, "true");
        //设置消息的生产组，在对于事务消息来说这很重要，因为当要commit或者rollback的时候，要通过PGROUP找到对应的生产者组
        ...
        switch (sendResult.getSendStatus()) {
            ...
            case SEND_OK: {
                try {
                    ...
                    // 记录发送消息返回的事务transactionId
                    String transactionId = msg.getProperty(MessageConst.PROPERTY_UNIQ_CLIENT_MESSAGE_ID_KEYIDX);
                    if (null != transactionId && !"".equals(transactionId)) {
                        msg.setTransactionId(transactionId);
                    }
                    //原来的方法中意思是用一个线程池去统一处理事务问题，新的接口中，localTransactionExecuter为null，不会进入
                    if (null != localTransactionExecuter) {
                        localTransactionState = localTransactionExecuter.executeLocalTransactionBranch(msg, arg);
                    } else if (transactionListener != null) {
                        //使用新的API：TransactionListener除了包括LocalTransactionExecuter的方法还附带了回查事务结果状态的功能，功能扩展了
