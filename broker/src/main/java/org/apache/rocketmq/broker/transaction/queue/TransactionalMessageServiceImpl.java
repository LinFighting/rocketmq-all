...
public class TransactionalMessageServiceImpl implements TransactionalMessageService {
    ...
    private boolean putBackHalfMsgQueue(MessageExt msgExt, long offset) {
        ...
        // 将该消息的queueOffset、commitLogOffset设置为重新存入的偏移量
    ...
    public void check(long transactionTimeout, int transactionCheckMax,
        ...
        try {
            ...
            for (MessageQueue messageQueue : msgQueues) {
                ...
                // 去RMQ_SYS_TRANS_HALF_TOPIC获取消息的offset
                long halfOffset = transactionalMessageBridge.fetchConsumeOffset(messageQueue);
                // 去RMQ_SYS_TRANS_OP_HALF_TOPIC获取消息的opOffset
                long opOffset = transactionalMessageBridge.fetchConsumeOffset(opQueue);
                log.info("Before check, the queue={} msgOffset={} opOffset={}", messageQueue, halfOffset, opOffset);
                // 两个队列都没获取到？？？todo 暂时没明白
                ...
                while (true) {
                    // 限制每次最多处理的时间，超时不处理（60s）
                    if (System.currentTimeMillis() - startTime > MAX_PROCESS_TIME_LIMIT) {
                        log.info("Queue={} process time reach max={}", messageQueue, MAX_PROCESS_TIME_LIMIT);
                        break;
                    }
                    //removeMap中 的值是通过fillOpRemoveMap中填充的，具体实现逻辑是从RMQ_SYS_TRANS_OP_HALF_TOPIC主题中拉取32条，
                    ...
                    if (removeMap.containsKey(i)) {
                        ...
                        long valueOfCurrentMinusBorn = System.currentTimeMillis() - msgExt.getBornTimestamp(); //该消息已存储的时间
                        long checkImmunityTime = transactionTimeout; //立即检测事务消息的时间
                        String checkImmunityTimeStr = msgExt.getUserProperty(MessageConst.PROPERTY_CHECK_IMMUNITY_TIME_IN_SECONDS);
                        //如果消息指定了事务消息过期时间属性，如果当前时间已超过该值，则跳出本次回查处理的，等下一次再试
